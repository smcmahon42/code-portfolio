...
define(['require', 'app/app'], function(require, app) {
	...
	app.controller('PromosSegmentCtrl', function($sce, $scope, apiSvc, $http) {
		var tempId,
			fileDataQueue = {},
			editDataQueue = {};
		...
		$scope.displayGlobalOptions = function(){
			...
			.success(function(data, status, headers, config){
			    ...
			    //$scope.globalOption.tipRepetitions = data.tipRepetitions;
			}).error(function(data){
		...
		$scope.setSegmentData = function(){
			if(Object.keys($scope.segmentForm).length === 2 &&
			  ...
			  ($scope.segmentForm.title !== "" && $scope.segmentForm.subtitle !== "") ){
				...
				apiSvc.call('POST', '/promos', { data: $scope.segmentForm } )
				.success(function(data, status, headers, config){
		...
		$scope.updatePublishType = function(e){
			...
			if( imageLocation !== null || audioLocation !== null ){
				...
				apiSvc.call('PUT', '/promos/'+id, { data: {"is_published" : bool} } )
				.success(function(data, status, headers, config){
					$scope.displaySegmentDatalist();
				}).error(function(data){
				...
				alert("Both an Image and Audio File needs to be set to this segment in order for it to be published.");
				return false;
			}
		}
		...
		$scope.uploadSegmentFile = function(){
			if(Object.keys(fileDataQueue).length === 2){

				for(var mediaType in fileDataQueue){
					$http.put(api_base+'/promos/'+tempId+'/upload/'+mediaType, fileDataQueue[mediaType], {
						headers: {'Content-Type': undefined,token: apiSvc.authToken },
						transformRequest: angular.identity
					}).success(function(){
						$scope.displaySegmentDatalist();
						$scope.resetSegmentDataForm();
					}).error(function(data){
				...
				alert("You must add both an audio and image file to finish the process.");
				return false;
			}
		}
		...
		$scope.editSegment = function(e){
			var titlesObj = {},
				rowId = e.segmentData.id;

			if(e.segmentData.title !== ''){ titlesObj['title'] = e.segmentData.title; }
			if(e.segmentData.subtitle !== ''){ titlesObj['subtitle'] = e.segmentData.subtitle; }
			if(titlesObj.title || titlesObj.subtitle){
				apiSvc.call('PUT', '/promos/'+rowId, { data: titlesObj } )
				.success(function(data, status, headers, config){
					$scope.displaySegmentDatalist();
			...
			for(var mediaType in editDataQueue){
				$http.put(api_base+'/promos/'+rowId+'/upload/'+mediaType, editDataQueue[mediaType], {
					headers: {'Content-Type': undefined,token: apiSvc.authToken },
					transformRequest: angular.identity
